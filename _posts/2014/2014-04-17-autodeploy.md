...
即使最简单的事情，重复几百、几千甚至几万次，必定有一次是错误的，除非是计算机。有一次，我的一位高中同学就告诉我，她那天多了收几毛钱，却连顾客和她都没有发现。要知道她从事收银的工作已经超过两年了，而且她告诉我，这种情况在她们同事看来是正常不过的。

和大多数的程序员一样，我不喜欢重复的工作，可是针对每个重复的事情，都像传说的自动化一样，编写一个工具，针对是一些周期比较短暂的工作，明显不合算。不过有一些是我的疏忽--就是部署项目。自动部署的话题两年前开始铺天盖地而来，实现上却没有完整的产品或方案。因为现在流行的解决方法有maven的cargo插件，ant、buildr等脚本，或者hudson插件，都不能在代理的环境下解决问题。
...
>   网路环境 开发环境和正式环境切开，不可直接通讯

比如hudson可以使用插件设置访问代理的配置，但也局限于http方式，所以实现自动部署时，也仅是通过tomcat的管理员账号将构建好的war进行部署。这样的代价是正式环境必须开放管理站点，可能带来安全隐患。

基于代理服务器可以连通外网的事实，我尝试使用git hooks来解决这个问题。
...
>   有四个钩子处理提交的流程。最新执行的是pre-commit钩子，它用来检查提交的快照，钩子返回非零值时，git放弃此次提交(可使用git commit
>   --no-verify来忽略)。
...
>   如果你和我一样，拷贝了.m2目录，建议下面的构建过程中使用mvn -o参数，实现离线构建的功能，以避免maven下载更多的其它依赖组件。

接着在代理服务器上，创建一个和github同名的本地仓库，初始化时有一点可以分享，就是--bare。对于分布式开发的服务器端版本库，要求使用git init
...
[2]: <https://github.com/51write/autodeploy.git>
...
另外，代码中还使用了线程变量Thread.current["mvn_msg"]，来实现线程与线程间的数据共享，这个特性比较类似map类型的操作。
...
原型中使用了大量的系统指令，以下为摘录的网络资料(源码中使用了第三种方式)。可见ruby实现脚本是非常有优势的。

>   1.Exec方法:   Kernel#exec方法通过调用指定的命令取代当前进程：   例子：  \$ irb
...
>   2.System方法。  Kernel#system方法操作命令同上，
>   但是它是运行一个子shell来避免覆盖当前进程。如果命令执行成功则返回true，否则返回false。   \$ irb  
...
>   这种方法是最普遍的用法了。它也是运行在一个子shell中。 4.IO#popen   \$ irb
